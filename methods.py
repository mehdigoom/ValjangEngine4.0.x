...
def detect_visual_c_compiler_version(tools_env):
    ...
    # There are other architectures, but ValjangEngine does not support them currently, so this function does not detect arm/amd64_arm
...
def generate_vs_project(env, num_jobs):
    ...
    if batch_file:
        ...
        debug_targets = ["bin\\ValjangEngine.windows.tools.32.exe"] + ["bin\\ValjangEngine.windows.tools.64.exe"]
        release_targets = ["bin\\ValjangEngine.windows.opt.32.exe"] + ["bin\\ValjangEngine.windows.opt.64.exe"]
        release_debug_targets = ["bin\\ValjangEngine.windows.opt.tools.32.exe"] + ["bin\\ValjangEngine.windows.opt.tools.64.exe"]
        targets = debug_targets + release_targets + release_debug_targets
        if not env.get("MSVS"):
            env["MSVS"]["PROJECTSUFFIX"] = ".vcxproj"
            env["MSVS"]["SOLUTIONSUFFIX"] = ".sln"
        env.MSVSProject(
            target=["#ValjangEngine" + env["MSVSPROJECTSUFFIX"]],
            incs=env.vs_incs,
            srcs=env.vs_srcs,
            runfile=targets,
            buildtarget=targets,
