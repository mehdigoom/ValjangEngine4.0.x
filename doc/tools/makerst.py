...
# http(s)://docs.ValjangEngineengine.org/<langcode>/<tag>/path/to/page.html(#fragment-tag)
ValjangEngine_DOCS_PATTERN = re.compile(
    r"^http(?:s)?://docs\.ValjangEngineengine\.org/(?:[a-zA-Z0-9.\-_]*)/(?:[a-zA-Z0-9.\-_]*)/(.*)\.html(#.*)?$"
)
...
def make_rst_class(class_def, state, dry_run, output_dir):  # type: (ClassDef, State, bool, str) -> None
    ...
    f.write(".. Generated automatically by doc/tools/makerst.py in ValjangEngine's source tree.\n")
...
def make_url(link):  # type: (str) -> str
    match = ValjangEngine_DOCS_PATTERN.search(link)
    if match:
        groups = match.groups()
