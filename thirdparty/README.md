...
Important: Some files have ValjangEngine-made changes.
They are marked with `// -- ValjangEngine start --` and `// -- ValjangEngine end --`
comments.
...
to be usable by ValjangEngine socket implementation and allow IPv6 and DTLS.
Apply the patches in the `patches/` folder when syncing on newer upstream
commits.

Two files (ValjangEngine.cpp and enet/ValjangEngine.h) have been added to provide
enet socket implementation using ValjangEngine classes.
...
the GLES version ValjangEngine targets.
...
ValjangEngine-made change marked with `// -- ValjangEngine --` comments.
...
Important: The files `utils/bit_reader_utils.{c,h}` have ValjangEngine-made
changes to ensure they build for Javascript/HTML5. Those
changes are marked with `// -- ValjangEngine --` comments.
...
- Added 2 files `ValjangEngine_core_mbedtls_platform.{c,h}` providing configuration
...
The only modified file is miniupnpcstrings.h, which was created for ValjangEngine
...
Important: Some files have ValjangEngine-made changes for use in core/io.
They are marked with `/* ValjangEngine start */` and `/* ValjangEngine end */`
...
Collection of single-file libraries used in ValjangEngine components.

### core

- `clipper.{cpp,hpp}`
  * Upstream: https://sourceforge.net/projects/polyclipping
  * Version: 6.4.2 (2017) + ValjangEngine changes (added optional exceptions handling)
  * License: BSL-1.0
- `cubemap_coeffs.h`
...
- `easing_equations.cpp`
  * Upstream: http://robertpenner.com/easing/ via https://github.com/jesusgollonet/ofpennereasing (modified to fit ValjangEngine types)
  * Version: git (af72c147c3a74e7e872aa28c7e2abfcced04fdce, 2008) + ValjangEngine types and style changes
  * License: BSD-3-Clause
- `mikktspace.{c,h}`
...
Modified files:
Modifications are marked with `// -- ValjangEngine start --` and `// -- ValjangEngine end --`.
...
Important: Some files have ValjangEngine-made changes; so to enrich the features
originally proposed by this library and better integrate this library with
ValjangEngine. Please check the file to know what's new.
...
`// -- ValjangEngine start --` and `// -- ValjangEngine end --` comments.
...
- `wslay.h` has a small ValjangEngine addition to fix MSVC build.
